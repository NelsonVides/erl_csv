...
- With the default separator and quote (`,` and `"`), and `"\n"` or `"\r\n"` as the delimiter,
  decoding takes another 3-38% less time, most on unquoted data: `decode/2` then uses a copy of
  the tokenizer that compares each byte with those constants instead of with the options it was
  given.
